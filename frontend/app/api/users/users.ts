...
export async function getUsers(
  ...
  try {
    const url = new URL(`http://backend:8080/v1/users`);
...
export async function getUsersAll(
  ...
  try {
    const url = new URL(`http://backend:8080/v1/users_all`);
...
export async function fetchLatestUsers() {
  try {
    const url = new URL(`http://backend:8080/v1/users_latest`);
    const searchParams = url.searchParams;
    searchParams.set("page_size", ITEMS_PER_PAGE.toString());
...
export async function getUser(id: string) {
  const url = new URL(`http://backend:8080/v1/users/${id}`);
