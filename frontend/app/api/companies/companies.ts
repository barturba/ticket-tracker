...
export async function getCompanies(
  ...
  try {
    const url = new URL(`http://backend:8080/v1/companies`);
...
export async function getCompaniesAll(
  ...
  try {
    const url = new URL(`http://backend:8080/v1/companies_all`);
...
export async function fetchLatestCompanies() {
  try {
    const url = new URL(`http://backend:8080/v1/companies_latest`);
    const searchParams = url.searchParams;
    searchParams.set("page_size", ITEMS_PER_PAGE.toString());
...
export async function getCompany(id: string) {
  const url = new URL(`http://backend:8080/v1/companies/${id}`);
