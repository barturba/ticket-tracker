...
export async function getCIs(
  ...
  try {
    const url = new URL(`http://${process.env.BACKEND}}/v1/cis`);
...
export async function getCIsAll(
  ...
  try {
    const url = new URL(`http://${process.env.BACKEND}/v1/cis_all`);
...
export async function fetchLatestCIs() {
  try {
    const url = new URL(`http://${process.env.BACKEND}/v1/cis_latest`);
    const searchParams = url.searchParams;
    searchParams.set("page_size", ITEMS_PER_PAGE.toString());
...
export async function getCI(id: string) {
  const url = new URL(`http://${process.env.BACKEND}/v1/cis/${id}`);
